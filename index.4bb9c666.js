...
class Game {
    ...
    move(transformRow) {
        ...
        this.state = this.state.map((row)=>{
            ...
            if (!this.arraysEqual(original, newRow)) changed = true;
            return newRow;
        });
        return changed;
    }
    ...
    updateStatus() {
        ...
        if (this.canMove()) this.status = "playing";
        else this.status = "lose";
    }
    canMove() {
        for(let r = 0; r < 4; r++)for(let c = 0; c < 4; c++){
            if (this.state[r][c] === 0) return true;
            if (c < 3 && this.state[r][c] === this.state[r][c + 1]) return true;
            if (r < 3 && this.state[r][c] === this.state[r + 1][c]) return true;
        }
        return false;
    }
    transpose(state) {
